...
![行式存储和列式存储](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/行式存储和列式存储.png)
...
![Redis 6之前IO模型](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis 6之前IO模型.png)
...
![Redis 6 IO模型](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis 6 IO模型.png)
...
![Redis执行事务](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis执行事务.png)
...
![Redis事务队列详情-执行前](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis事务队列详情-执行前.png)

命令被执行后，也会生成一个结果数组，Redis就将这个结果数组返回:

![Redis事务队列详情-执行后.png](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis事务队列详情-执行后.png)
...
![Redis事务错误-EXEC执行前](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis事务错误-EXEC执行前.png)

- EXEC执行命令时出现错误: EXEC执行命令时出现错误，也就是用户发出的命令没有错，
  但是在执行命令的时候出现了错误(可能是参数类型不对)，这时候仍然返回结果数组,
  也就是说错误的命令并不影响其他命令的执行。

![Redis事务错误-EXEC执行时](E:/尚硅谷java/JVMzhouyang/framework-learning/img/database/redis/Redis事务错误-EXEC执行时.png)
...
Redis的内存会和Swap(虚拟内存)频繁切换，使得Redis性能下降。
